...
export default function BookingDetail() {
    ...
    return (
      <Layout>
        <div className="container mx-auto px-4 py-24 text-center">
          ...
          <Button asChild><Link href="/bookings">Back to my bookings</Link></Button>
        </div>
      </Layout>
    );
  }
  ...
  const getIcon = (type: string) => {
    switch (type) {
      ...
      case 'package': return <Map className="w-8 h-8" />;
      ...
      <div className="bg-muted/20 pb-12 pt-6 border-b">
        <div className="container mx-auto px-4 max-w-5xl">
          <Button variant="ghost" asChild className="mb-6 pl-0 hover:bg-transparent">
            <Link href="/bookings" className="text-muted-foreground flex items-center">
          ...
          <div className="flex flex-col md:flex-row gap-6 justify-between items-start md:items-center bg-card p-6 md:p-8 rounded-2xl shadow-sm border">
            <div className="flex items-center gap-6">
              <div className={`p-4 rounded-2xl ${booking.status === 'cancelled' ? 'bg-muted text-muted-foreground' : 'bg-primary/10 text-primary'}`}>
              ...
              <div>
                <div className="flex items-center gap-3 mb-2">
                  ...
                  {booking.status === 'pending' && <Badge variant="secondary" className="bg-yellow-500/20 text-yellow-700">Pending</Badge>}
                  {booking.status === 'cancelled' && <Badge variant="destructive">Cancelled</Badge>}
                </div>
                <h1 className="text-2xl md:text-3xl font-extrabold">
                  {booking.details?.title as string || `Booking Reference: ${booking.referenceId}`}
                </h1>
                <p className="text-muted-foreground mt-1">
                  Booked on {booking.createdAt ? format(new Date(booking.createdAt), "MMMM do, yyyy") : 'Unknown Date'}
                </p>
              </div>
            </div>
                  ...
                  <AlertDialogContent>
                    <AlertDialogHeader>
                      ...
                      <AlertDialogDescription>
                        ...
                        A cancellation confirmation email will be sent to {booking.passengerEmail}.
                      </AlertDialogDescription>
                    </AlertDialogHeader>
                    <AlertDialogFooter>
      ...
      <div className="container mx-auto px-4 py-12 max-w-5xl">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <Card className="shadow-sm border-muted">
            <CardHeader className="bg-muted/10 border-b pb-4">
              <CardTitle className="text-lg flex items-center">
                <Users className="w-5 h-5 mr-2 text-primary" /> Passenger Information
              </CardTitle>
            </CardHeader>
            <CardContent className="pt-6 space-y-6">
              <div>
                <p className="text-sm text-muted-foreground font-medium mb-1">Primary Contact</p>
              ...
              <div className="grid grid-cols-2 gap-6">
                <div>
                  ...
                  <p className="font-medium truncate">{booking.passengerEmail}</p>
                </div>
                <div>
                  <p className="text-sm text-muted-foreground font-medium mb-1">Phone</p>
                  <p className="font-medium">{booking.passengerPhone || "Not provided"}</p>
                </div>
              </div>
              <div>
                <p className="text-sm text-muted-foreground font-medium mb-1">Total Passengers</p>
          ...
          <Card className="shadow-sm border-muted">
            <CardHeader className="bg-muted/10 border-b pb-4">
              <CardTitle className="text-lg flex items-center">
                <CreditCard className="w-5 h-5 mr-2 text-primary" /> Payment Summary
              </CardTitle>
            </CardHeader>
            <CardContent className="pt-6 space-y-6">
              <div>
                <p className="text-sm text-muted-foreground font-medium mb-1">Booking Reference ID</p>
              ...
              <div className="border-t border-dashed pt-4 space-y-3">
                <div className="flex justify-between text-sm">
                  <span className="text-muted-foreground">Base Price ({booking.passengers}x)</span>
            ...
            <div className="md:col-span-2 bg-green-500/10 border border-green-500/20 rounded-xl p-6 flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-green-700/80 text-sm leading-relaxed">
                  Your booking is confirmed. A detailed itinerary and receipt have been sent to {booking.passengerEmail}. 
                  Please keep this reference handy and present it upon arrival.
                </p>
              </div>
            </div>
          )}
            ...
            <div className="md:col-span-2 bg-destructive/10 border border-destructive/20 rounded-xl p-6 flex items-start gap-4">
              ...
              <div>
                ...
                <p className="text-destructive/80 text-sm leading-relaxed">
                  This booking has been cancelled. If a refund is applicable per the provider's policy, it will be processed to your original payment method within 5-7 business days.
                </p>
              </div>
            </div>
          )}
